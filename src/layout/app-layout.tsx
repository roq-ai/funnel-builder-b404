...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<FunnelBuilderInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/funnel-builders` : null),
    () => getFunnelBuilders({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Funnel Builder',
      path: isTenantUser ? `/funnel-builders/view/${data?.[0]?.id}` : '/funnel-builders',
      entity: 'funnel_builder',
      service: AccessServiceEnum.PROJECT,
      icon: FiLayers,
    },
    { name: 'Block', path: '/blocks', entity: 'block', service: AccessServiceEnum.PROJECT, icon: FiBox },
    {
      name: 'Integration',
      path: '/integrations',
      entity: 'integration',
