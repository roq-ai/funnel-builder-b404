...
export const getIntegrations = async (query?: IntegrationGetQueryInterface) => {
  const response = await axios.get(`/api/integrations${query ? `?${queryString.stringify(query)}` : ''}`);
