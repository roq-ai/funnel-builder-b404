...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getBlockById() {
    const data = await prisma.block.findFirst(convertQueryToPrismaUtil(req.query, 'block'));
    return res.status(200).json(data);
  }

  async function updateBlockById() {
    await blockValidationSchema.validate(req.body);
